...
public class CourseRecordIO {
	...
	private static Course readCourse(String nextLine) {
		String[] courseDetails = nextLine.split(",");
		Course returned = null;
		boolean valid = true;
		
		//look for with and without meeting times
		if (courseDetails.length != courseStringLength - 2 && courseDetails.length != courseStringLength) {
			valid = false;
		}
		
		//without meeting times
		if (courseDetails.length == courseStringLength - 2 && valid) {
			returned = new Course(
					courseDetails[0], 
					courseDetails[1], 
					courseDetails[2],
					...
					courseDetails[4],
					Integer.parseInt(courseDetails[5]), 
					courseDetails[6]
					);
		} else if (courseDetails.length == courseStringLength && valid) {
			returned = new Course(
					courseDetails[0], 
					courseDetails[1], 
					courseDetails[2],
					...
					Integer.parseInt(courseDetails[5]), 
					courseDetails[6],
					Integer.parseInt(courseDetails[7]),
					Integer.parseInt(courseDetails[8])
					);
		}
