 ...
 * 
 * @author William Boyles, Joey Davidson, and Matthew Strickland
 */
public class RegistrationManagerTest {
	...
	public void testGetCourseCatalog() {
		// Reference to empty catalog from manager
		CourseCatalog cat = manager.getCourseCatalog();
		// check that it's empty
		assertTrue(cat.getCourseCatalog().length == 0);
		// add a course
		cat.addCourseToCatalog("CSC216", "Programming Concepts - Java", "001", 4, "semwith5", 200, "MW", 1330, 1445);
		// check that something was added.
		assertTrue(cat.getCourseCatalog().length == 1);
		// make a duplicate course to the one added
		Course c = new Course("CSC216", "Programming Concepts - Java", "001", 4, "semwith5", 200, "MW", 1330, 1445);
	...
	public void testGetStudentDirectory() {
		// reference to empty directory from manager
		StudentDirectory dir = manager.getStudentDirectory();
		// check that it's empty
		assertTrue(dir.getStudentDirectory().length == 0);
		// add a student
		dir.addStudent("William", "Boyles", "wmboyle2", "wmboyle2@ncsu.edu", "password", "password", 18);
		// check that something was added
		assertTrue(dir.getStudentDirectory().length == 1);
		// create a matching student
		Student w = new Student("William", "Boyles", "wmboyle2", "wmboyle2@ncsu.edu", "password", 18);
		// check that they are the same
		Student gotId = dir.getStudentById("wmboyle2");
		assertEquals(gotId.getFirstName(), w.getFirstName());
		assertEquals(gotId.getLastName(), w.getLastName());
		assertEquals(gotId.getEmail(), w.getEmail());
		assertEquals(gotId.getId(), w.getId());
		assertEquals(gotId.getMaxCredits(), w.getMaxCredits());
	...
	public void testLogin() {
		...
		// add a student
		dir.addStudent("William", "Boyles", "wmboyle2", "wmboyle2@ncsu.edu", "password", "password", 18);
		// login using valid student credentials
		assertTrue(manager.login("wmboyle2", "password"));
		// login using invalid student credentials
		assertFalse(manager.login("wmboyle2", "notPassword"));

		manager.logout();

		// login using valid registrar credentials
		...
		assertTrue(manager.login(p.getProperty("id"), p.getProperty("pw")));
		manager.logout();
		// login using invalid registrar credentials for a bad id
		try {
			assertFalse(manager.login("badRegistrar", "badPassword"));
		} catch (IllegalArgumentException e) {
			assertEquals("User doesn't exist.", e.getMessage());
		}
		// login using invalid registrar credentials for a bad password
		try {
			assertFalse(manager.login(p.getProperty("id"), "badPassword"));
		} catch (IllegalArgumentException e) {
		...
		// Trying to replicate Jenkins test
		manager.clearData();
		dir.addStudent("Emerald", "Frost", "efrost", "adipiscing@acipsumPhasellus.edu", "password", "password", 3);
		assertTrue(manager.login("efrost", "password"));
		assertFalse(manager.login("efrost", "password"));
		assertEquals(1, dir.getStudentDirectory().length);
		assertFalse(manager.login("efrost", "password"));
	}
	...
	public void testLogout() {
		...
		assertTrue(manager.login("wmboyle2", "password"));
		// get current user
		User currentLoggedIn = manager.getCurrentUser();
		// logout and see that the current user changed
		manager.logout();
	...
	public void testGetCurrentUser() {
		...
		// get current user
		User currentLoggedIn = manager.getCurrentUser();
		// check that the user changed when logged in
		assertFalse(currentLoggedIn == null);
		// logout and check that the user changed again
		manager.logout();
		assertFalse(currentLoggedIn.equals(manager.getCurrentUser()));
	}
	...
	/**
	 * This tests login in as a faculty
	 */
	@Test
	...
	/**
	 * Tests addFacultyToCourseMethod
	 */
	@Test
	public void testAddFacultyToCourseMethod() {
		manager.logout();
		try {
			manager.resetSchedule();
			manager.addFacultyToCourse(new Course("Name", "Title", "Section", 3, "sesmith", 40, "MWF", 1040, 1130), new Faculty("FirstName", "LastName", "sesmith", "sesmith@ncsu.edu", "password", 3));
			fail();
		} catch (IllegalArgumentException e) {
